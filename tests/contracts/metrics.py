...
SANCTIONED_EXTENSIONS = {
    ...
    "covariate_is_required_for_scalar_metrics": (
        "`covariate=None` is REJECTED by mutual_information, clonotypic_entropy and "
        "phenotypic_entropy. It is still accepted by joint_distribution.\n"
        "\n"
        "It used to stack every covariate level into one table, so a clone present in k levels "
        "contributed k ROWS and the row axis of the joint became the (covariate, clone) pair "
        "rather than the clone. H(c) was then the entropy over pseudo-clones. Measured on a "
        "10-clone / 2-covariate fixture:\n"
        "\n"
        "    C=20 P=4  (many clones)   min  +0.0%   average +13.8%\n"
        "    C=6  P=8  (few clones)    min +12.4%   average +15.4%\n"
        "\n"
        "`min` divides by min(H(c), H(phi)). In the REALISTIC regime for a repertoire -- "
        "clones vastly outnumbering phenotypes -- it selects H(phi), which row-splitting does "
        "not touch, so the default is genuinely unaffected. The C=6/P=8 row is included only "
        "to show the exemption is a property of that ratio and not of the metric: an earlier "
        "revision of this entry stated 'invisible at the default' as though it were universal. "
        "`average` moves in every regime, including the realistic one.\n"
        "\n"
        "The three reducing metrics also disagreed on what covariate=None MEANT: clonotypic "
        "collapsed to a per-phenotype index, phenotypic kept a (covariate, clonotype) index, "
        "and mutual_information stacked. Picking one unification is a question about the "
        "ESTIMAND, not about the code, and the metrics document does not settle it -- so the "
        "argument is required rather than resolved by guess. Revisit deliberately if a "
        "covariate-marginal estimand is ever specified.\n"
        "\n"
        "joint_distribution is exempt because it LABELS the blocks with a covariate index "
        "level instead of collapsing them: no reduction, no ambiguity, and it stays the way to "
        "get every level in one object."
    ),
    ...
    "store_once_payload": (
        ...
        "\n"
        "Two uncertainty families coexist and are deliberately named apart: `hdi_*` on "
        "`result` is the WITHIN-group posterior interval over draws; `ci_*`/`sd_*`/`n_*` on "
        "`stats` is the BETWEEN-replicate spread of each arm, across groups. They answer "
    ...
    "the_delta_family": (
        ...
        "defines them; the note defines quantities at a covariate level, not changes between "
        "levels. Recorded here because they are new estimands, not new estimators.\n"
        "\n"
        "Delta = value(cov_to) - value(cov_from), computed WITHIN a posterior draw, so what is "
        "summarized is the distribution of the difference rather than the difference of two "
        "summaries. HDIs do not subtract: the interval on a delta cannot be recovered from the "
        "intervals on its endpoints.\n"
        "\n"
        "ONLY metrics with an item axis get one. clonotypic_entropy reduces to a phenotype and "
        "phenotypic_entropy to a clonotype, so their deltas need the metric applied inside the "
        "engine's draw loop with identity aligned across two covariate blocks. "
        "mutual_information has no item axis -- it is already the repertoire-level number, so "
        "its 'delta' is a subtraction of two cached scalars and belongs to the caller. See the "
        "scope principle in the API contract.\n"
        "\n"
        "SUPPORT: the clone set is the INTERSECTION of clones present at both levels, taken "
        "WITHIN each replicate. A delta needs both endpoints, exactly as phenotypic_flux "
        "already requires. Two consequences. For phenotypic_entropy the intersection decides "
        "which rows exist; for clonotypic_entropy it constrains the clone set summed over "
        "inside H(c|phi), which makes log2(C) identical on both sides so the normalizer "
        "cancels -- without it, a repertoire contracting 150 -> 90 clones reports "
        "+0.078 normalized entropy having not redistributed at all. And the drop moves n, so "
        "it is warned about rather than silently applied.\n"
        "\n"
        "NO direction probability. P(delta > 0) is computable from the cached `table` in one "
        "line; it is not a column because it reads as a frequentist p-value, its resolution is "
        "capped at 1/n_samples (so 1.0 means 'no draw crossed zero', not certainty), and "
        "per-item it invites filtering on the survivors. `hdi_low`/`hdi_high` already answer "
        "direction in the Bayesian idiom."
    ),
    "the_contrast_is_a_rank_test_over_groups": (
        "`stats` is a two-sided Mann-Whitney U on the per-GROUP values, with `stars` at the "
        "conventional 0.05/0.01/0.001/0.0001 thresholds. Neither document specifies a test: "
        "the metrics document defines quantities, not comparisons, so this is ours.\n"
        "\n"
        "Rank-based rather than a t-test because the metrics are bounded ([0,1] normalized, "
        "[0,2] for L1 flux) and n is the patient count, so normality is neither given nor "
        ...
        "so that choice is no longer available to get wrong."
    ),
}
