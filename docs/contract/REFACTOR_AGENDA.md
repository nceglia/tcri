...
- **Deferred (author sign-off — change fitted results):** **[E]** `reconstruction_loss_scale=1e-3` (eq 7 is full-weight; over-generation symptom); **[G]** α (`global_scale`) not applied to the eq-1 clonotype prior (prior/guide scale mismatch). **[F]** in-silico perturbation (eqs 8–12) not implemented (additive).
