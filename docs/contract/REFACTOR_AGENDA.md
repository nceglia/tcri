...
- **Removed (hard bar):** **NONE YET** — the removal wins (dedup the 3 console copies; move `stars`/`auc`/`bootstrap` out of `utils`; the 2 `dkl` copies → `_distance`; replace literal `tcri_*` keys with `K.*`) are the ADOPTION step, still pending. Ledger stays unticked until then.
