...
def _compute_logits_and_prior_from_z(model, adata, z, batch_size=4096, eps=1e-8):
    """
    Compute logits = classifier(z) and prior_log = log p_ct[ct_arr] *without* re-encoding.
    Expects z as numpy float32 array from get_latent_representation.
    """
    import torch
    import numpy as np

    device = next(model.module.parameters()).device
    model.module.eval()  # ensure dropout etc. are disabled

    # ct indices per cell (already stored by register_model step 2)
    if "tcri_ct_array_for_cells" in adata.uns:
        ct_arr_np = adata.uns["tcri_ct_array_for_cells"]
    else:
        ct_arr_np = model.module.ct_array.cpu().numpy()
    ct_arr = torch.as_tensor(ct_arr_np, device=device, dtype=torch.long)

    # robust p_ct retrieval (works even if pyro store was missing and got seeded)
    try:
        p_ct = model.module.get_p_ct().to(device)
    except KeyError:
        # uniform fallback
        # infer shapes
        ct_count = int(getattr(model.module, "ct_count", int(ct_arr.max().item() + 1)))
        with torch.no_grad():
            _logits_probe = model.module.classifier(torch.from_numpy(z[:1]).to(device))
        P = int(_logits_probe.shape[-1])
        p_ct = torch.full((ct_count, P), 1.0 / P, device=device)

    logits_buf, prior_buf = [], []
    with torch.inference_mode():
        for i in range(0, z.shape[0], batch_size):
            z_b = torch.from_numpy(z[i:i + batch_size]).to(device)
            logits = model.module.classifier(z_b)
            prior_log = torch.log(p_ct[ct_arr[i:i + batch_size]] + eps)
            logits_buf.append(logits.detach().cpu())
            prior_buf.append(prior_log.detach().cpu())

    cls_logits = torch.cat(logits_buf, dim=0).numpy().astype("float32")
    prior_log = torch.cat(prior_buf, dim=0).numpy().astype("float32")
    return cls_logits, prior_log
...
#     clonotype_key="trb_unique",
#     # --- posterior controls ---
#     prior_weight: float = 1.0,      # ↑ pull closer to prior
#     logit_weight: float = 1.0,      # ↓ dampen model evidence
#     temperature: float = 1.0,       # T>1 flattens → relatively more prior
#     overwrite_probs: bool = True,   # recompute if slot already exists
#     # --- performance knob ---
#     fast: bool = True,              # reuse z to avoid a second encoder pass
# ):
#     import numpy as np
#     import pandas as pd
#     from scipy.special import softmax
...
#     # 4) logits & log-posterior -----------------------------------------
#     if fast:
#         cls_logits, prior_log = _compute_logits_and_prior_from_z(model, adata, z, batch_size=max(batch_size, 2048))
#     else:
#         # legacy path (re-encodes the data: slower)
#         cls_logits, prior_log = _compute_logits_and_prior(model, adata, batch_size)
