...
SANCTIONED_EXTENSIONS = {
    ...
    "covariate_is_required_for_scalar_metrics": (
        ...
        "\n"
        "`min` divides by min(H(c), H(phi)). When clones OUTNUMBER phenotypes it selects "
        "H(phi), which row-splitting does not touch, so the default looks unaffected; when "
        "they do not it selects H(c) and moves by ~12%. An earlier revision of this entry "
        "claimed the defect was 'invisible at the default' -- that was a property of one "
        "fixture (20 clones, 4 phenotypes), not of the metric. `average` always moves.\n"
        "\n"
