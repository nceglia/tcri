...
highest-density interval, NaN at `n_samples ≤ 1`); `stats` is `None` without `splitby`. Every
...
**Diagnostics.** `joint_distribution_ppc`, `phenotype_calibration` and `permutation_null` are
model-free and read the substrate; `reconstruction_ppc`, `loss` and `archetypes` take the
